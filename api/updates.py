...
def _check_repo(path, name):
    ...
    # Use the current branch's upstream tracking ref, not the repo default.
...
def _apply_update_inner(target):
    ...
    # Use the current branch's upstream for pull, matching _check_repo behaviour.
    # Falls back to default branch if no upstream is set.
    upstream, ok = _run_git(['rev-parse', '--abbrev-ref', '@{upstream}'], path)
    if ok and upstream and '/' in upstream:
        # upstream is like "origin/feat/foo" — split into remote + refspec
        remote, ref = upstream.split('/', 1)
    else:
        remote = 'origin'
        ref = _detect_default_branch(path)
    ...
    pull_out, pull_ok = _run_git(['pull', '--ff-only', remote, ref], path, timeout=30)
    if not pull_ok:
        if stashed:
